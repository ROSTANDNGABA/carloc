...
def vehicule_disponible_pour_periode(
    vehicule, date_debut, date_fin, exclude_reservation_pk=None, lock=False
) -> bool:
    """
...
@transaction.atomic
def creer_facture(
    reservation, type_facture="location", paiement=None, montant=None
) -> Facture:
    ...
    if type_facture == "location":
        facture_existante = (
            reservation.factures.filter(type_facture="location")
...
@transaction.atomic
def finaliser_reservation(
    reservation: Reservation, date_retour=None, kilometrage_retour=None
) -> Contrat:
    from .pdf_generators import generer_pdf_contrat
...
def annuler_reservation(reservation: Reservation) -> Reservation:
    ...
    reservation.factures.filter(statut__in=("emise", "brouillon")).update(
        statut="annulee"
    )
    sync_vehicule_statut(reservation.vehicule)
...
def peut_annuler_reservation(reservation, user=None) -> bool:
    ...
    if reservation.est_annulee:
        raise ReservationCancellationError(
            "Cette réservation est déjà annulée.", code="already_cancelled"
        )
...
@transaction.atomic
def annuler_reservation_avec_regles(
    reservation, user=None, ip_address=None, raison=""
) -> dict:
    """
    ...
    montant_remboursé, montant_pénalité, taux_raison = (
        calculer_remboursement_annulation(reservation)
    )
