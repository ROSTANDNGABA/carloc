...
class GestionnaireViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return User.objects.filter(is_staff=True, is_superuser=False).order_by(
    ...
    def list(self, request, *args, **kwargs):
        ...
        for user in users:
            ...
            data.update(
                stats.get(user.id, {"chiffre_affaires": 0, "locations_realisees": 0})
            )
            payload.append(data)
        if page is not None:
    ...
    def _stats_by_manager(self, include_history=False):
        ...
        for row in paiement_logs:
            paiement_ids_by_actor.setdefault(row["actor_id"], set()).add(
                row["object_id"]
            )
        ...
        for row in reservation_logs:
            reservation_ids_by_actor.setdefault(row["actor_id"], set()).add(
                row["object_id"]
            )
        ...
        for manager in self.get_queryset():
            ...
            chiffre_affaires = (
                Paiement.objects.filter(pk__in=paiement_ids).aggregate(
...
class VehiculeViewSet(viewsets.ModelViewSet):
    ...
    def disponibilite(self, request, pk=None):
        ...
        message = (
            ...
            if disponible
            else ("Véhicule indisponible (maintenance ou réservation existante).")
        )
...
class ClientViewSet(viewsets.ModelViewSet):
    ...
    def historique(self, request, pk=None):
        ...
        if not user.is_staff and (
            ...
            return Response(
                {"detail": "Accès non autorisé."}, status=status.HTTP_403_FORBIDDEN
            )
...
class PaiementViewSet(viewsets.ModelViewSet):
    queryset = Paiement.objects.select_related(
        "reservation__client", "reservation__vehicule"
    )
...
class ContratViewSet(viewsets.ModelViewSet):
    ...
    def cloturer(self, request, pk=None):
        ...
        date_retour = (
            input_serializer.validated_data.get("date_retour") or timezone.now().date()
        )
...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            "date_debut", str, OpenApiParameter.QUERY, description="YYYY-MM-DD"
        ),
        OpenApiParameter(
            "date_fin", str, OpenApiParameter.QUERY, description="YYYY-MM-DD"
        ),
    ],
