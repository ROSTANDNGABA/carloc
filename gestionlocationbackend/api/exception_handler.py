...
def carloc_exception_handler(exc, context):
    ...
    if isinstance(exc, DjangoValidationError):
        return _build_response(
            ...
            'validation_error',
            'Données invalides.',
            _normalize_details(exc.message_dict if hasattr(exc, 'message_dict') else exc.messages),
        )
    ...
    if response is not None:
        ...
        message = details.get('detail') or details.get('non_field_errors') or 'Requête invalide.'
