...
def format_whatsapp_number(telephone: str) -> str:
    ...
    elif not cleaned.startswith('237') and len(cleaned) == 9: # Numéro local camerounais sans le 0
...
def envoyer_message_whatsapp(telephone: str, message: str) -> bool:
    ...
    if not client:
        logger.warning("Twilio n'est pas configuré. Le message WhatsApp n'a pas pu être envoyé.")
        return False
    ...
    try:
        ...
        logger.info(f"Message WhatsApp envoyé avec succès au {to_number} (SID: {msg.sid})")
        return True
    except TwilioRestException as e:
        logger.error(f"Erreur Twilio lors de l'envoi WhatsApp au {to_number}: {e}")
        return False
    except Exception as e:
        logger.exception(f"Erreur inattendue lors de l'envoi WhatsApp au {to_number}: {e}")
        return False
