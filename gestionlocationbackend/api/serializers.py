...
class CarLocTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def _resolve_django_username(raw_identifier: str) -> str:
        ...
        prenom_qs = Client.objects.filter(
            prenom__iexact=raw, user_id__isnull=False
        ).select_related("user")
    ...
    def validate(self, attrs):
        ...
        if hasattr(self.user, "client_profile"):
            ...
            photo_profil = (
                self.user.client_profile.photo_profil.url
                if self.user.client_profile.photo_profil
                else None
            )
...
class GestionnaireSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        username = (
            attrs.get("username") or getattr(self.instance, "username", "")
        ).strip()
        email = (
            (attrs.get("email") or getattr(self.instance, "email", "")).strip().lower()
        )
...
class VehiculeSerializer(serializers.ModelSerializer):
    ...
    def validate_prix_journalier(self, value):
        if value <= 0:
            raise serializers.ValidationError(
                "Le prix journalier doit être strictement positif."
            )
        return value
    ...
    def validate_immatriculation(self, value):
        ...
        if instance:
            if (
                Vehicule.objects.filter(immatriculation=value)
...
class ClientSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if solde is not None and solde < 0:
            raise serializers.ValidationError(
                {"solde": "Le solde ne peut pas etre negatif."}
            )
        ...
        if self.instance is None:
            ...
            if email and Client.objects.filter(email__iexact=email).exists():
                raise serializers.ValidationError(
                    {"email": "Un client existe deja avec cet email."}
                )
            if (
                num_permis
                and Client.objects.filter(num_permis__iexact=num_permis).exists()
            ):
                raise serializers.ValidationError(
    ...
    def update(self, instance, validated_data):
        ...
        if (
            email
            and User.objects.filter(username=email)
...
class ReservationSerializer(serializers.ModelSerializer):
    ...
    montant_penalites = serializers.DecimalField(
        max_digits=12, decimal_places=2, read_only=True, coerce_to_string=False
    )
    montant_du = serializers.DecimalField(
        max_digits=12, decimal_places=2, read_only=True, coerce_to_string=False
    )
    ...
    def validate(self, attrs):
        ...
        if client and debut and fin:
            exclude_pk = instance.pk if instance else None
            valider_eligibilite_reservation(
                client, debut, fin, exclude_reservation_pk=exclude_pk
            )
...
class PaiementSerializer(serializers.ModelSerializer):
    ...
    reservation_solde_restant = serializers.DecimalField(
        source="reservation.solde_restant",
        max_digits=12,
        decimal_places=2,
        read_only=True,
    )
    ...
    class Meta:
        ...
        fields = [
            ...
            "date_paiement",
            "nom_client",
            "reservation_montant_du",
            "reservation_solde_restant",
        ]
        read_only_fields = ["date_paiement"]

    def validate(self, attrs):
        reservation = attrs.get("reservation") or getattr(
            self.instance, "reservation", None
        )
        montant = attrs.get(
            "montant_paye", getattr(self.instance, "montant_paye", None)
        )
...
class MaintenanceSerializer(serializers.ModelSerializer):
    immatriculation_vehicule = serializers.CharField(
        source="vehicule.immatriculation", read_only=True
    )
    ...
    def validate(self, attrs):
        ...
        date_operation = attrs.get("date_operation") or getattr(
            self.instance, "date_operation", None
        )
...
class ContratSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        instance = self.instance
        km_depart = attrs.get(
            "kilometrage_depart", getattr(instance, "kilometrage_depart", None)
        )
        km_retour = attrs.get(
            "kilometrage_retour", getattr(instance, "kilometrage_retour", None)
        )
