...
class PermissionsTests(CarLocTestCase):
    ...
    def test_historique_client_autre_refuse(self):
        ...
        autre_user = User.objects.create_user(
            "autre@test.com", "autre@test.com", "password12345"
        )
    ...
    def test_dashboard_admin_seulement(self):
        self.client.force_authenticate(user=self.client_user)
        self.assertEqual(
            self.client.get("/api/dashboard/").status_code, status.HTTP_403_FORBIDDEN
        )

        self.client.force_authenticate(user=self.admin)
        self.assertEqual(
            self.client.get("/api/dashboard/").status_code, status.HTTP_200_OK
        )
