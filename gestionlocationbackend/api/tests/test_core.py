...
from api.services import calculer_penalites_retard, creer_contrat_pour_reservation, creer_facture
...
class CoreMetierTests(CarLocTestCase):
    ...
    def test_client_modifie_son_profil_et_user_est_synchronise(self):
        ...
        response = self.client.patch(f'/api/clients/{self.client_profile.id}/', {
            'nom': 'Nouveau',
            'prenom': 'Profil',
    ...
    def test_reservation_cree_contrat_facture_et_notification(self):
        ...
        self.assertTrue(Facture.objects.filter(reservation=reservation, type_facture='location').exists())
        self.assertTrue(Contrat.objects.get(reservation=reservation).fichier_pdf)
        self.assertTrue(NotificationLog.objects.filter(reservation=reservation).exists())
    ...
    def test_paiement_valide_et_acompte(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(
            Facture.objects.filter(reservation=reservation, type_facture='acompte').exists()
        )
    ...
    def test_penalites_retard(self):
        ...
        self.assertEqual(penalites, Decimal('5') * self.vehicule.prix_journalier * Decimal('1.5'))
