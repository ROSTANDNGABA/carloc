...
class TestAnnulationEdgeCases:
    ...
    def test_annulation_exactement_48h_avant(self, client_user, vehicule):
        ...
        assert '100%' in result['taux_remboursement'] or '>48h' in result['taux_remboursement']
    ...
    def test_annulation_exactement_24h_avant(self, client_user, vehicule):
        ...
        ConfigurationMetier.objects.get_or_create(
            key='REFUND_RATE_24H',
            defaults={'value_decimal': Decimal('0.8')}
        )
    ...
    def test_annulation_moins_24h_admin_autorise(self, admin_user, client_user, vehicule):
...
class TestReservationConcurrency:
    ...
    def test_deux_clients_reservent_meme_vehicule_simultanément(self, vehicule):
        ...
        assert reservations_crees == 1, f"Expected 1 reservation, got {reservations_crees}"

    def test_modification_reservation_pendant_chevauchement(self, client_user, vehicule):
        ...
        serializer = ReservationSerializer(
            resa2,
            data={
                'date_debut': timezone.now().date() + timedelta(days=2),  # Chevauche resa1
        ...
        assert 'vehicule' in serializer.errors or 'déjà réservé' in str(serializer.errors)
