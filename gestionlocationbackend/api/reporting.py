    ...
    for vehicule in vehicules:
        ...
        for resa in vehicule.reservations_filtrees:
            ...
            if date_debut or date_fin:
                ...
                revenus += sum(
                    (p.montant_paye for p in resa.paiements.all()), Decimal("0")
                )
...
def statistiques_clients(date_debut=None, date_fin=None):
    ...
    clients_actifs = (
        Client.objects.filter(reservations__est_annulee=False).distinct().count()
    )
    ...
    impayes = []
    clients_avec_reservations = (
        Client.objects.prefetch_related("reservations__paiements")
        .filter(reservations__est_annulee=False, reservations__date_fin__lt=now.date())
        .distinct()
    )
...
def _compute_dashboard_complet(date_debut=None, date_fin=None):
    ...
    taux_occupation = (
        round((vehicules_loues / total_vehicules) * 100, 1) if total_vehicules else 0
    )
    ...
    reservations_impayees = sum(
        1
        for r in Reservation.objects.filter(est_annulee=False).prefetch_related(
            "paiements"
        )
...
def historique_vehicule(vehicule):
    ...
    maintenances = Maintenance.objects.filter(vehicule=vehicule).order_by(
        "-date_operation"
    )
...
def historique_client(client):
    ...
    factures = Facture.objects.filter(reservation__client=client).order_by(
        "-date_emission"
    )
        ...
        "resume": {
            ...
            "solde_impaye": sum(
                (
                    r.solde_restant
                    for r in reservations
                    if not r.est_annulee and r.solde_restant > 0
                ),
                Decimal("0"),
            ),
        },
