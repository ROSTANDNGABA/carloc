...
def _styles():
    ...
    return {
        'title': ParagraphStyle('Title', parent=base['Heading1'], fontSize=16, spaceAfter=12),
        'normal': base['Normal'],
        'small': ParagraphStyle('Small', parent=base['Normal'], fontSize=9, textColor=colors.grey),
    }
...
def generer_pdf_contrat(contrat: Contrat) -> Contrat:
    ...
    elements = [
        ...
        Paragraph(f'Contrat n° {contrat.id} — Réservation n° {reservation.id}', styles['normal']),
        Spacer(1, 0.5 * cm),
        Paragraph(f'<b>Client :</b> {client.prenom} {client.nom} — {client.email}', styles['normal']),
        Paragraph(f'<b>Téléphone :</b> {client.telephone} — <b>Permis :</b> {client.num_permis}', styles['normal']),
        Spacer(1, 0.3 * cm),
        Paragraph(
    ...
    if contrat.kilometrage_depart is not None:
        elements.append(Paragraph(f'Kilométrage départ : {contrat.kilometrage_depart} km', styles['normal']))
    if contrat.kilometrage_retour is not None:
        elements.append(Paragraph(f'Kilométrage retour : {contrat.kilometrage_retour} km', styles['normal']))
...
def generer_pdf_facture(facture: Facture) -> Facture:
    ...
    elements = [
        ...
        Paragraph(f'Date d\'émission : {facture.date_emission:%d/%m/%Y %H:%M}', styles['normal']),
        Spacer(1, 0.5 * cm),
        Paragraph(f'<b>Client :</b> {client.prenom} {client.nom}', styles['normal']),
        Paragraph(f'<b>Email :</b> {client.email} — <b>Tél :</b> {client.telephone}', styles['normal']),
        Spacer(1, 0.3 * cm),
        Paragraph(
    ...
    if facture.type_facture == 'location':
        data.append(['Déjà payé (acomptes, etc.)', _fmt_montant(reservation.total_paye)])
    ...
    elements.append(table)
    elements.append(Spacer(1, 1 * cm))
    elements.append(Paragraph('Merci pour votre confiance. CarLoc — Location de véhicules.', styles['small']))
