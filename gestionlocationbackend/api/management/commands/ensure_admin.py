...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--username", default=os.environ.get("CARLOC_ADMIN_USERNAME", default_email)
        )
        parser.add_argument("--email", default=default_email)
        parser.add_argument(
            "--password", default=os.environ.get("CARLOC_ADMIN_PASSWORD")
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Compte administrateur {action}: {user.username}")
        )
