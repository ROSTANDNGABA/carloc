...
def record_http_request(method: str, view: str, status: str, duration_seconds: float) -> None:
    HTTP_REQUESTS_TOTAL.labels(method=method, view=view, status=status).inc()
    HTTP_REQUEST_DURATION_SECONDS.labels(method=method, view=view).observe(duration_seconds)
